...
## The systemd service — generated by setup.sh, nowhere else

`setup.sh` writes `/etc/systemd/system/kitchensync.service` with the CURRENT user,
paths, and `.venv` python, `ExecStartPre=tools/start_x.sh`, `Restart=always`. It is
the ONLY source of truth — a stale browser-era unit file was tracked in the repo
until 2026-07-07 and misled people; it has been deleted.
...
`tools/reset-network.sh` is a chrony-era artifact (ntp-setup.sh was deleted
2026-07-07) — sync does NOT need NTP (see ksync-failure-archaeology E7).

## Provenance and maintenance

Written 2026-07-06. Re-verify:
- setup.sh package list / service template: `grep -n "apt install\|tee /etc/systemd" setup.sh`
- No unit file has crept back into the repo: `git ls-files | grep -c "\.service$"` (expect 0)
